class IntcodeParser:
    ...
    def get_index(self, intcode, param_mode, pos, rel_base):
        ...
        if self.param_mode == 0:
            self.index = int(intcode[pos]) 
        elif self.param_mode == 1:
            self.index = int(pos)
        elif self.param_mode == 2:
            self.index = int(intcode[pos]) + rel_base
        return self.index

    def write_value_to_index(self, intcode, index, value):
        if len(intcode) <= index:
            self.length_needed = (index - len(intcode))+1
            intcode = intcode + [0] * self.length_needed
        intcode[index] = value
        return intcode

    def read_value_from_index(self, intcode, index, ret_int=True):
        if len(intcode) <= index:
    ...
    def parse_intcode(
        ...
        while True:
            ...
            pointer = str(self.read_value_from_index(intcode, pos)[0])
            ...
            if instruction in [1, 2]:
                ...
                index_1 = self.get_index(intcode, param_1_mode, pos+1, rel_base)
                index_2 = self.get_index(intcode, param_2_mode, pos+2, rel_base)
                index_3 = self.get_index(intcode, param_3_mode, pos+3, rel_base)
                
                param_1, intcode = self.read_value_from_index(intcode, index_1)
                param_2, intcode = self.read_value_from_index(intcode, index_2)

                result =  (param_1 + param_2) if instruction == 1 else (param_1 * param_2)

                intcode = self.write_value_to_index(intcode, index_3, str(result))
                pos += 4
            ...
            ### INS: 3 ###
                ...
                index = self.get_index(intcode, param_1_mode, pos+1, rel_base)

                intcode = self.write_value_to_index(intcode, index, value)
            ...
            ### INS: 4 ###
                ...
                index = self.get_index(intcode, param_1_mode, pos+1, rel_base)
                param_1, intcode = self.read_value_from_index(intcode, index)
            ...
            ### INS: 7, 8 ###
                ...
                index_1 = self.get_index(intcode, param_1_mode, pos+1, rel_base)
                index_2 = self.get_index(intcode, param_2_mode, pos+2, rel_base)
                index_result = self.get_index(intcode, param_3_mode, pos+3, rel_base)
                ...
                if instruction == 7:
                    ...
                    intcode = self.write_value_to_index(intcode, index_result, result)
